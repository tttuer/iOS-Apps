...
//                longitudeCenter: 2.21656815003280, // 127
    ...
    func convert(sourceType: MapProjectionType, destinationType: MapProjectionType, geoPoint: GeographicPoint) -> GeographicPoint? {        
        let sourcePoint = ({ () -> GeographicPoint? in
            if sourceType == .WGS_84 {
                return GeographicPoint(longitude: degreeToRadian(geoPoint.x), latitude: degreeToRadian(geoPoint.y))
            } else {
        ...
        let destinationPoint = ({ () -> GeographicPoint? in
            if destinationType == .WGS_84 {
                return GeographicPoint(longitude: radianToDegree(sourcePoint!.x), latitude: radianToDegree(sourcePoint!.y))
            } else {
    ...
    func wgs84ToGrid(_ point: GeographicPoint) -> GeographicPoint? {
        ...
        let wgs84Point = GeographicPoint(longitude: (ra * sin(theta)) + mapInfo.xo, latitude: (ro - ra * cos(theta)) + mapInfo.yo)
        
        return GeographicPoint(longitude: floor(wgs84Point.x + 1.5), latitude: floor(wgs84Point.y + 1.5))
    }
    ...
    private func geodeticToTm(destination: MapProjectionType, inputPoint: GeographicPoint) -> GeographicPoint? {
        ...
        return GeographicPoint(longitude: x, latitude: y)
    }
    ...
    private func tmToGeodetic(source: MapProjectionType, inputPoint: GeographicPoint) -> GeographicPoint? {
        let newPoint = GeographicPoint(
            longitude: inputPoint.x - geoCoordDatas[source]!.falseEasting,
            latitude: inputPoint.y - geoCoordDatas[source]!.falseNorthing)
        let maxIter = 6
